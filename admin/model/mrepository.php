<?php
class mrepository extends Database {
...
// pr($query);
	...
	{	
		...
		$sql = array(
                'table'=>"{$this->prefix}_news_content_repo",
                'field'=>"title, typealbum, gallerytype, created_date, otherid, n_status, files",
                'value' => "'{$title}','{$typealbum}', '{$gallerytype}', '{$created_date}', '0', '{$n_status}', '{$files}'",
                );
	...
	function updateGalleryRepo($data, $debug=false)
	{
		...
		$sql = array(
                'table'=>"{$this->prefix}_news_content_repo",
                'field'=>"title = '{$title}', n_status = {$n_status}",
                'condition' => "id = {$id}",
                );
