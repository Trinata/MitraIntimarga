...
class repository extends Controller {
	...
	public function loadmodule()
	{
		...
		$this->marticle = $this->loadModel('mrepository');
	...
	public function index(){
		...
		// if ($data){
			...
			// foreach ($data as $key => $val){
				...
				$dataAlbum = $this->marticle->getRepo($id=false, $type=1,$cat=1, 0);
		...
		return $this->loadView('news/home-repository');
	...
	function addFile()
	{
	    		...
	    		if(!empty($_FILES)){
					if($_FILES['file_image']['name'] != ''){
						...
						if ($image['status']){
							...
							// pr($saveData);
							if ($saveData){
								echo "<script>alert('Data berhasil di simpan');window.location.href='".$basedomain."repository'</script>";
							}	
						}
		...
		return $this->loadView('news/add-repository');
	}
	...
	public function articledel(){
		...
		echo "<script>alert('Data has been moved to trash');window.location.href='".$CONFIG['admin']['base_url']."repository'</script>";
